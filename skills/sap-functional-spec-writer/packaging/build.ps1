<#
.SYNOPSIS
    Builds and validates the distribution artifacts for sap-functional-spec-writer.
...
if ($text -notmatch 'Never write a rule nobody decided') {
    Fail "the core no-invention rule is missing from SKILL.md 2"
}
foreach ($cm in 'CONFIRMADO', 'A VALIDAR', 'EM ABERTO') {
    if (-not $text.Contains($cm)) { Fail "confidence mark '$cm' is missing from SKILL.md" }
}
if (-not $text.Contains('Q-###')) { Fail "open-point identifier Q-### is missing from SKILL.md" }
SecOk "no-invention discipline and confidence marks present"

# --- 5. boundaries with the sibling skills ------------------------------------
Mark
# All three cover SAP functional work and will compete for the same phrasing.
foreach ($sib in 'sap-functional-test-productivity', 'sap-functional-status-report') {
    if (-not $text.Contains($sib)) { Fail "SKILL.md does not declare its boundary with $sib" }
}
if ($text -notmatch '(?i)authors|authoring|writes') {
    Fail "SKILL.md must state that it authors rather than analyzes"
}
SecOk "sibling boundaries declared"
...
if (-not $Check) {
    ...
    $corePrompt = @(
        ...
        $core
        ''
        'Knowledge files carry the depth. `spec-template.md` defines the document structure.'
    ) -join "`n"
    $corePrompt += "`n"
